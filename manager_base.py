...
MEME_PROMPT_MARKER_END = "<!-- meme_manager_master_prompt:end -->"
SEMANTIC_PROMPT_MARKER_START = "<!-- meme_manager_master_semantic_prompt:start -->"
SEMANTIC_PROMPT_MARKER_END = "<!-- meme_manager_master_semantic_prompt:end -->"
...
class MemeSender(Star, WebAPIMixin, CommandMixin, EventHandlerMixin):
    ...
    def _resolve_runtime_pack_context(
        ...
        if event is not None and hasattr(event, "set_extra"):
            event.set_extra(
                "meme_manager_master_runtime_memes_dir",
                str(context.get("memes_dir", MEMES_DIR)),
            )
            event.set_extra(
                "meme_manager_master_runtime_pack_id", str(context.get("pack_id") or "")
            )
        return context
    ...
    def _get_runtime_memes_dir_for_event(self, event: AstrMessageEvent):
        if hasattr(event, "get_extra"):
            runtime_memes_dir = str(
                event.get_extra("meme_manager_master_runtime_memes_dir") or ""
            ).strip()
            if runtime_memes_dir:
                return runtime_memes_dir
        try:
            return str(get_active_pack_paths()["memes_dir"])
        except Exception:
        ...
        if self.emotion_llm_enabled:
            ...
            if semantic_tool_ready:
                ...
                if semantic_probability_hit:
                    semantic_mode = "tool"
                logger.info(
                    "[meme_manager_master] 语义表情触发判定: 概率=%s%%, 结果=%s",
                    normalize_probability(self.emotions_probability),
                    "命中" if semantic_probability_hit else "跳过",
                )
        semantic_active = bool(semantic_mode)
        if event is not None and hasattr(event, "set_extra"):
            event.set_extra("meme_manager_master_semantic_active", semantic_active)
            event.set_extra("meme_manager_master_semantic_mode", semantic_mode)
            event.set_extra(
                "meme_manager_master_semantic_probability_hit",
                semantic_probability_hit,
            )
            event.set_extra(
                "meme_manager_master_semantic_verified_pack_id",
                str(event.get_extra("meme_manager_master_runtime_pack_id") or "")
                if semantic_active
                else "",
            )
            event.set_extra("meme_manager_master_semantic_selected_ids", [])
            event.set_extra("meme_manager_master_semantic_candidates", {})
            event.set_extra("meme_manager_master_semantic_query", "")
            event.set_extra("meme_manager_master_semantic_search_completed", False)
            event.set_extra("meme_manager_master_semantic_default_id", "")
            event.set_extra("meme_manager_master_semantic_response_processed", False)
            event.set_extra("meme_manager_master_reply_provider_id", "")
            event.set_extra("meme_manager_master_reply_model", "")
    ...
    def _reload_personas(self):
        ...
        self.category_mapping = runtime_category_mapping(
            ...
            if isinstance(context_mapping, dict)
                ...
                get_active_pack_paths().get("metadata_path", MEMES_DATA_PATH),
                DEFAULT_CATEGORY_DESCRIPTIONS,
            )
        )
