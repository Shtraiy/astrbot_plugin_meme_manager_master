...
管理页面支持分类浏览、图片上传、图片预览、删除、移动、分类描述编辑和 pack 切换；表情索引页在已整理区与待处理区之间提供独立分页，并支持跨已整理分页累积选择。普通管理页的删除行为不写入捕获黑名单。自动选图只按主分类建立候选集合，图片语义字段仅用于模型判断；旧目录无法无歧义推断主分类时会标记为 `needs_reindex`，需要点击“全量语义重索引”完成语义重建后才会进入自动发送候选。
