...
管理页面使用单页布局，通过 `#manage`、`#index`、`#settings` 切换表情管理、表情索引和设置中心。页面首次打开时加载全部静态资源，后续切换不再请求新的 HTML；同时支持分类浏览、单图上传、预览、删除、移动、分类描述编辑、pack 切换、索引整理、选择规则与备份迁移。
...
`pack.js`、`emoji.js`、三个视图入口与 `router.js`，共享命名空间
