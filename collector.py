...
MEME_CAPTURE_RUBRIC = """
表情包主要用于聊天中表达情绪、态度、反应、吐槽或玩梗，而不是传递图片本身的信息。
以下正向证据满足越多，越像表情包：有明显人物、动漫角色或动物主体并表现出明确情绪或动作；有短文字、字幕或梗文字；构图简单、主体突出；适合作为对上一句话的反应；脱离原始场景后仍可重复用于其他聊天；属于常见 meme、reaction image、二次元表情或熊猫头等。
以下情况通常不是表情包，必须显著扣分或拒绝：普通照片、自拍、风景；游戏截图、软件或网页截图、聊天记录截图；二维码、商品图；教程图、数据图、报错截图；海报、广告或横幅；单纯的动漫截图或影视截图且没有明显表情包用途；主要为了传递具体信息的图片。
用户消息语境只能作为辅助证据，不能把信息图、截图或普通照片升级为表情包。无法确认时宁可拒绝。
请输出 meme_score，范围为 0–100，表示这张图片作为聊天表情包使用的可能性。
""".strip()
