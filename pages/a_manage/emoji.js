...
window.MemeManagerUI.emoji.semanticReviewKey = function (category, filename) {
    return `${String(category || "")}\u0000${String(filename || "")}`;
  }
window.MemeManagerUI.emoji.semanticReviewLabel = function (status) {
    return (
      {
        auto_match: "自动符合",
        needs_review: "建议人工复核",
...
window.MemeManagerUI.emoji.updateSemanticReviewToolbar = function () {
    if (!window.MemeManagerUI.state.semanticReviewStats) return;
    window.MemeManagerUI.state.semanticReviewStats.querySelectorAll("button[data-review-filter]").forEach((button) => {
      const filter = String(button.dataset.reviewFilter || "all");
      const count =
        filter === "all"
          ? Number(window.MemeManagerUI.state.semanticReviewStatistics.total || 0)
          : Number(window.MemeManagerUI.state.semanticReviewStatistics[filter] || 0);
      const value = button.querySelector("span");
      if (value) value.textContent = String(count);
      button.classList.toggle("active", filter === window.MemeManagerUI.state.activeSemanticReviewFilter);
    });
  }
window.MemeManagerUI.emoji.applySemanticReviewData = function (payload) {
    window.MemeManagerUI.state.semanticReviewAvailable = Boolean(payload?.available);
    const items =
      window.MemeManagerUI.state.semanticReviewAvailable && Array.isArray(payload?.items)
        ? payload.items
        : [];
    window.MemeManagerUI.state.semanticReviewByPath = new Map(
      items.map((item) => [
        window.MemeManagerUI.emoji.semanticReviewKey(item?.category, item?.filename),
        item,
      ]),
    );
    window.MemeManagerUI.state.semanticReviewStatistics = window.MemeManagerUI.state.semanticReviewAvailable
      ? payload?.statistics || {}
      : {};
    if (!window.MemeManagerUI.state.semanticReviewAvailable) {
      window.MemeManagerUI.state.activeSemanticReviewFilter = "all";
    }
    window.MemeManagerUI.state.semanticReviewToolbar?.classList.toggle("hidden", !window.MemeManagerUI.state.semanticReviewAvailable);
...
window.MemeManagerUI.emoji.renderImageSemantic = function (semantic, { loading = false, error = "" } = {}) {
    ...
    if (window.MemeManagerUI.state.imagePreviewState && !loading && !error) {
      window.MemeManagerUI.state.imagePreviewState.semantic = semantic || { status: "none" };
    }
    statuses.forEach((item) => {
      window.MemeManagerUI.state.imagePreviewSemantic.classList.toggle(
        `semantic-${item}`,
        item === normalizedStatus,
      );
    });

    if (window.MemeManagerUI.state.imagePreviewSemanticFilename) {
      window.MemeManagerUI.state.imagePreviewSemanticFilename.textContent = window.MemeManagerUI.state.imagePreviewState?.emoji || "";
    }
    ...
    window.MemeManagerUI.state.imagePreviewCategoryReview?.classList.toggle(
      "hidden",
      !showCategoryReview,
    );
    if (window.MemeManagerUI.state.imagePreviewCategoryTag) {
      window.MemeManagerUI.state.imagePreviewCategoryTag.textContent = loading
        ? "读取固定标签…"
        : String(
            semantic?.category_tag ||
              `category:${window.MemeManagerUI.state.imagePreviewState?.category || ""}`,
          );
    }
    if (window.MemeManagerUI.state.imagePreviewCategoryReviewState) {
      window.MemeManagerUI.state.imagePreviewCategoryReviewState.textContent = loading
        ? "读取审核状态…"
        : window.MemeManagerUI.emoji.semanticReviewLabel(reviewStatus);
    }
    const reviewReason = String(semantic?.category_review_reason || "").trim();
    if (window.MemeManagerUI.state.imagePreviewCategoryReviewReason) {
      window.MemeManagerUI.state.imagePreviewCategoryReviewReason.textContent = reviewReason;
      window.MemeManagerUI.state.imagePreviewCategoryReviewReason.classList.toggle(
        "hidden",
    ...
    if (window.MemeManagerUI.state.imagePreviewReclassification) {
      ...
      window.MemeManagerUI.state.imagePreviewReclassification.textContent = reclassificationStatus
        ? `自动重分类：${fromCategory || "原分类"} → ${toCategory || "当前分类"}${
            reclassificationReason ? `；${reclassificationReason}` : ""
          }`
        : "";
      window.MemeManagerUI.state.imagePreviewReclassification.classList.toggle(
        "hidden",
        loading || !reclassificationStatus,
      );
    }
    window.MemeManagerUI.state.imagePreviewCategoryConfirmBtn?.classList.toggle(
      "hidden",
      !canEdit || !semantic?.can_confirm_category,
    );
    window.MemeManagerUI.state.imagePreviewReviewEditBtn?.classList.toggle(
      "hidden",
    ...
    window.MemeManagerUI.state.imagePreviewReviewActions?.classList.toggle("hidden", !showReviewActions);
    const manualModified = Boolean(semantic?.manual_modified);
    if (window.MemeManagerUI.state.imagePreviewSourceState) {
      window.MemeManagerUI.state.imagePreviewSourceState.textContent = manualModified
        ? "人工修改"
        : "自动生成";
      window.MemeManagerUI.state.imagePreviewSourceState.classList.toggle("status-manual", manualModified);
    }
    const embeddingStatus = String(semantic?.embedding_status || "pending");
    if (window.MemeManagerUI.state.imagePreviewVectorState) {
      const vectorLabels = {
        done: "向量更新完成",
        failed: "向量更新失败",
        running: "向量更新中",
      ...
      window.MemeManagerUI.state.imagePreviewVectorState.textContent = loading
        ? "读取向量状态…"
        : vectorLabels[embeddingStatus] || "向量等待更新";
      window.MemeManagerUI.state.imagePreviewVectorState.classList.remove(
        "status-done",
        "status-failed",
        "status-pending",
      );
      window.MemeManagerUI.state.imagePreviewVectorState.classList.add(
    ...
    const duplicatePaths = Array.isArray(semantic?.same_content_paths)
      ...
      : [];
    if (window.MemeManagerUI.state.imagePreviewDuplicateNotice) {
      window.MemeManagerUI.state.imagePreviewDuplicateNotice.textContent = duplicatePaths.length
        ? `检测到另外 ${duplicatePaths.length} 张内容相同的图片。本次只修改当前路径，不会改动：${duplicatePaths.join("、")}`
        : "";
      window.MemeManagerUI.state.imagePreviewDuplicateNotice.classList.toggle(
        "hidden",
    ...
    window.MemeManagerUI.state.imagePreviewSemanticActions?.classList.toggle(
      "hidden",
      !showBottomActions,
    );
    window.MemeManagerUI.state.imagePreviewEditBtn?.classList.toggle(
      "hidden",
      !canEdit || needsHumanReview,
    );
    window.MemeManagerUI.state.imagePreviewRestoreAutoBtn?.classList.toggle(
      "hidden",
      !canEdit || !semantic?.can_restore_auto,
    );
    if (loading) {
      window.MemeManagerUI.state.imagePreviewSemanticState.textContent = "读取中";
      window.MemeManagerUI.state.imagePreviewSemanticCaption.textContent = "正在读取这张图片的语义信息…";
    } else if (error) {
      window.MemeManagerUI.state.imagePreviewSemanticState.textContent = "读取失败";
      window.MemeManagerUI.state.imagePreviewSemanticCaption.textContent = error;
    } else if (normalizedStatus === "complete") {
      window.MemeManagerUI.state.imagePreviewSemanticState.textContent = "已语义化";
      window.MemeManagerUI.state.imagePreviewSemanticCaption.textContent =
        String(semantic?.caption || "").trim() || "已有语义记录。";
    } else if (normalizedStatus === "failed") {
      window.MemeManagerUI.state.imagePreviewSemanticState.textContent = "语义化失败";
      window.MemeManagerUI.state.imagePreviewSemanticCaption.textContent =
        String(semantic?.error || "").trim() ||
        "这张图片上次语义化失败，可以前往语义化页面重试。";
    } else if (normalizedStatus === "partial") {
      window.MemeManagerUI.state.imagePreviewSemanticState.textContent = "等待完成";
      window.MemeManagerUI.state.imagePreviewSemanticCaption.textContent =
        "这张图片已进入语义化流程，但描述尚未完成。";
    } else {
      window.MemeManagerUI.state.imagePreviewSemanticState.textContent = "未语义化";
      window.MemeManagerUI.state.imagePreviewSemanticCaption.textContent =
        "这张图片还没有语义描述，可以前往语义化页面继续处理。";
    }

    const tags = Array.isArray(semantic?.tags) ? semantic.tags : [];
    if (window.MemeManagerUI.state.imagePreviewSemanticTags) {
      window.MemeManagerUI.state.imagePreviewSemanticTags.replaceChildren();
      tags.forEach((tag) => {
        const chip = document.createElement("span");
        chip.textContent = String(tag);
        window.MemeManagerUI.state.imagePreviewSemanticTags.appendChild(chip);
      });
    }
    window.MemeManagerUI.state.imagePreviewSemanticTagsWrap?.classList.toggle(
      "hidden",
      loading || tags.length === 0,
    );

    const visibleText = String(semantic?.visible_text || "").trim();
    if (window.MemeManagerUI.state.imagePreviewVisibleText) {
      window.MemeManagerUI.state.imagePreviewVisibleText.textContent = visibleText;
    }
    window.MemeManagerUI.state.imagePreviewVisibleTextWrap?.classList.toggle(
      "hidden",
      loading || !visibleText,
    );

    if (window.MemeManagerUI.state.imagePreviewSemanticIndex) {
      window.MemeManagerUI.state.imagePreviewSemanticIndex.textContent =
...
window.MemeManagerUI.emoji.setImageSemanticEditing = function (editing) {
    if (!window.MemeManagerUI.state.imagePreviewSemantic || !window.MemeManagerUI.state.imagePreviewEditForm) return;
    window.MemeManagerUI.state.imagePreviewSemantic.classList.toggle("editing", editing);
    window.MemeManagerUI.state.imagePreviewEditForm.classList.toggle("hidden", !editing);
    if (!editing) return;
    const semantic = window.MemeManagerUI.state.imagePreviewState?.semantic || {};
    if (window.MemeManagerUI.state.imagePreviewCaptionInput) {
      window.MemeManagerUI.state.imagePreviewCaptionInput.value = String(semantic.caption || "");
    }
    if (window.MemeManagerUI.state.imagePreviewTagsInput) {
      const tags = Array.isArray(semantic.editable_tags)
        ? semantic.editable_tags
        : [];
      window.MemeManagerUI.state.imagePreviewTagsInput.value = tags.join("，");
    }
    if (window.MemeManagerUI.state.imagePreviewVisibleTextInput) {
      window.MemeManagerUI.state.imagePreviewVisibleTextInput.value = String(semantic.visible_text || "");
    }
    if (window.MemeManagerUI.state.imagePreviewCategoryDecision) {
      window.MemeManagerUI.state.imagePreviewCategoryDecision.value =
    ...
    if (window.MemeManagerUI.state.imagePreviewFixedTags) {
      window.MemeManagerUI.state.imagePreviewFixedTags.replaceChildren();
      ...
      fixedTags.forEach((tag) => {
        const chip = document.createElement("span");
        chip.textContent = String(tag);
        window.MemeManagerUI.state.imagePreviewFixedTags.appendChild(chip);
      });
    }
    if (window.MemeManagerUI.state.imagePreviewReviewInstruction) {
      window.MemeManagerUI.state.imagePreviewReviewInstruction.value = "";
    }
    if (window.MemeManagerUI.state.imagePreviewReviewRewriteStatus) {
      window.MemeManagerUI.state.imagePreviewReviewRewriteStatus.textContent =
        "模型会重写语义并自动选择分类；只生成候选，不会直接保存或移动。";
      window.MemeManagerUI.state.imagePreviewReviewRewriteStatus.classList.remove("has-proposal");
    }
    if (window.MemeManagerUI.state.imagePreviewTargetCategory) {
      window.MemeManagerUI.state.imagePreviewTargetCategory.replaceChildren();
      const placeholder = document.createElement("option");
      placeholder.value = "";
      placeholder.textContent = "保持当前分类";
      window.MemeManagerUI.state.imagePreviewTargetCategory.appendChild(placeholder);
      const currentCategory = String(window.MemeManagerUI.state.imagePreviewState?.category || "");
      const categories = Array.from(
        new Set([
          ...Object.keys(window.MemeManagerUI.state.latestEmojiData || {}),
          ...Object.keys(window.MemeManagerUI.state.latestTagDescriptions || {}),
        ]),
      )
      ...
      categories.forEach((category) => {
        ...
        window.MemeManagerUI.state.imagePreviewTargetCategory.appendChild(option);
      });
      window.MemeManagerUI.emoji.updateImageSemanticMoveChoice();
    }
    const duplicateCount = Number(semantic.same_content_count || 0);
    if (window.MemeManagerUI.state.imagePreviewEditScope) {
      window.MemeManagerUI.state.imagePreviewEditScope.textContent = duplicateCount
        ? `只修改当前路径。检测到另外 ${duplicateCount} 张同内容图片，它们不会被一并修改。`
        : "只修改当前图片路径和当前分类，不会批量共享修改。";
    }
    window.setTimeout(() => window.MemeManagerUI.state.imagePreviewCaptionInput?.focus(), 0);
  }
window.MemeManagerUI.emoji.parseManualSemanticTags = function (value) {
    return Array.from(
      new Set(
        String(value || "")
          .split(/[，,\n]+/)
...
window.MemeManagerUI.emoji.imageSemanticSnapshotPayload = function () {
    const state = window.MemeManagerUI.state.imagePreviewState;
    const semantic = state?.semantic || {};
    if (
      !state ||
      String(window.MemeManagerUI.state.activeManagePackId || "") !== String(state.packId || "")
    ) {
...
window.MemeManagerUI.emoji.imageSemanticEditPayload = function () {
    const caption = String(window.MemeManagerUI.state.imagePreviewCaptionInput?.value || "").trim();
    if (!caption) {
    ...
    return {
      ...
      caption,
      tags: window.MemeManagerUI.emoji.parseManualSemanticTags(window.MemeManagerUI.state.imagePreviewTagsInput?.value),
      visible_text: String(window.MemeManagerUI.state.imagePreviewVisibleTextInput?.value || "").trim(),
      category_decision: String(window.MemeManagerUI.state.imagePreviewCategoryDecision?.value || "keep"),
      target_category: String(window.MemeManagerUI.state.imagePreviewTargetCategory?.value || "").trim(),
    };
  }
window.MemeManagerUI.emoji.updateImageSemanticMoveChoice = function () {
    const targetCategory = String(
      window.MemeManagerUI.state.imagePreviewTargetCategory?.value || "",
    ).trim();
    const saving = window.MemeManagerUI.state.imagePreviewEditForm?.dataset.saving === "true";
    if (window.MemeManagerUI.state.imagePreviewSaveBtn && !saving) {
      window.MemeManagerUI.state.imagePreviewSaveBtn.disabled = Boolean(targetCategory);
      window.MemeManagerUI.state.imagePreviewSaveBtn.title = targetCategory
        ? "选择新分类后，需要同时更新该图向量"
        : "只保存语义，向量等待后续更新";
    }
    if (window.MemeManagerUI.state.imagePreviewSaveVectorBtn && !saving) {
      window.MemeManagerUI.state.imagePreviewSaveVectorBtn.disabled = false;
      window.MemeManagerUI.state.imagePreviewSaveVectorBtn.textContent = targetCategory
        ? "保存、移动并更新该图向量"
        : "保存并更新该图向量";
    }
  }
window.MemeManagerUI.emoji.setImageSemanticSaving = function (
    saving,
    activeButton = null,
    busyLabel = "保存中...",
  ) {
    if (window.MemeManagerUI.state.imagePreviewEditForm) {
      window.MemeManagerUI.state.imagePreviewEditForm.dataset.saving = saving ? "true" : "false";
    }
    [
      window.MemeManagerUI.state.imagePreviewEditCancelBtn,
      window.MemeManagerUI.state.imagePreviewSaveBtn,
      window.MemeManagerUI.state.imagePreviewSaveVectorBtn,
      window.MemeManagerUI.state.imagePreviewEditBtn,
      window.MemeManagerUI.state.imagePreviewReviewEditBtn,
      window.MemeManagerUI.state.imagePreviewRestoreAutoBtn,
      window.MemeManagerUI.state.imagePreviewCategoryConfirmBtn,
      window.MemeManagerUI.state.imagePreviewReviewRewriteBtn,
    ].forEach((button) => {
      if (button) button.disabled = saving;
    });
    [
      window.MemeManagerUI.state.imagePreviewCaptionInput,
      window.MemeManagerUI.state.imagePreviewTagsInput,
      window.MemeManagerUI.state.imagePreviewVisibleTextInput,
      window.MemeManagerUI.state.imagePreviewCategoryDecision,
      window.MemeManagerUI.state.imagePreviewReviewInstruction,
      window.MemeManagerUI.state.imagePreviewTargetCategory,
    ].forEach((field) => {
      if (field) field.disabled = saving;
    });
    if (saving && activeButton) {
      window.MemeManagerUI.emoji.setButtonBusy(activeButton, busyLabel);
    } else if (!saving && activeButton) {
      window.MemeManagerUI.emoji.restoreButton(activeButton);
    }
    if (!saving) {
      window.MemeManagerUI.emoji.updateImageSemanticMoveChoice();
    }
  }
window.MemeManagerUI.emoji.requestImageSemanticRevision = async function () {
    ...
    if (!reviewInstruction) {
      ...
      window.MemeManagerUI.state.imagePreviewReviewInstruction?.focus();
      return;
    }
    try {
      const payload = {
      ...
      window.MemeManagerUI.emoji.setImageSemanticSaving(
        true,
        window.MemeManagerUI.state.imagePreviewReviewRewriteBtn,
        "视觉模型分析中...",
      );
      ...
      if (window.MemeManagerUI.state.imagePreviewCaptionInput) {
        window.MemeManagerUI.state.imagePreviewCaptionInput.value = String(proposal.caption || "");
      }
      if (window.MemeManagerUI.state.imagePreviewTagsInput) {
        window.MemeManagerUI.state.imagePreviewTagsInput.value = Array.isArray(proposal.tags)
          ? proposal.tags.join("，")
          : "";
      }
      if (window.MemeManagerUI.state.imagePreviewVisibleTextInput) {
        window.MemeManagerUI.state.imagePreviewVisibleTextInput.value = String(
      ...
      if (window.MemeManagerUI.state.imagePreviewCategoryDecision) {
        if (
          ...
          window.MemeManagerUI.state.imagePreviewCategoryDecision.value = "match";
        } else if (
          ["return_original", "move_to_other"].includes(
            classificationAction,
          ) || categoryFit === "conflict"
        ) {
          window.MemeManagerUI.state.imagePreviewCategoryDecision.value = "mismatch";
        } else {
          window.MemeManagerUI.state.imagePreviewCategoryDecision.value = "keep";
        }
      }
      ...
      if (window.MemeManagerUI.state.imagePreviewTargetCategory) {
        ...
        const hasTargetOption = Array.from(
          window.MemeManagerUI.state.imagePreviewTargetCategory.options,
        ).some((option) => option.value === selectableTarget);
        targetOptionAvailable = !selectableTarget || hasTargetOption;
        window.MemeManagerUI.state.imagePreviewTargetCategory.value = hasTargetOption
          ? selectableTarget
          : "";
        window.MemeManagerUI.emoji.updateImageSemanticMoveChoice();
      }
      if (window.MemeManagerUI.state.imagePreviewReviewRewriteStatus) {
        ...
        window.MemeManagerUI.state.imagePreviewReviewRewriteStatus.textContent = [
          ...
          .filter(Boolean)
          .join(" ");
        window.MemeManagerUI.state.imagePreviewReviewRewriteStatus.classList.add("has-proposal");
      }
      window.MemeManagerUI.dialogs.showToast(
      ...
      window.MemeManagerUI.dialogs.showToast(error?.message || String(error), "error", "重写失败", 4800);
    } finally {
      window.MemeManagerUI.emoji.setImageSemanticSaving(false, window.MemeManagerUI.state.imagePreviewReviewRewriteBtn);
    }
  }
window.MemeManagerUI.emoji.saveCurrentImageSemantic = async function ({ updateVector = false } = {}) {
    const activeButton = updateVector
      ? window.MemeManagerUI.state.imagePreviewSaveVectorBtn
      : window.MemeManagerUI.state.imagePreviewSaveBtn;
    const previewState = window.MemeManagerUI.state.imagePreviewState;
    try {
      const payload = window.MemeManagerUI.emoji.imageSemanticEditPayload();
      ...
      if (window.MemeManagerUI.state.imagePreviewState === previewState) {
        if (result?.moved) {
          ...
          if (window.MemeManagerUI.state.imagePreviewImg) {
            window.MemeManagerUI.state.imagePreviewImg.alt = `表情包预览：${previewState.emoji}`;
          }
        }
        window.MemeManagerUI.emoji.renderImageSemantic(result?.semantic || {});
      ...
      window.MemeManagerUI.dialogs.showToast(error?.message || String(error), "error", "保存失败");
    } finally {
      window.MemeManagerUI.emoji.setImageSemanticSaving(false, activeButton);
    }
  }
window.MemeManagerUI.emoji.restoreCurrentImageAutoSemantic = async function () {
    const state = window.MemeManagerUI.state.imagePreviewState;
    const semantic = state?.semantic || {};
    ...
    if (!confirmed) return;
    try {
      if (String(window.MemeManagerUI.state.activeManagePackId || "") !== String(state.packId || "")) {
      ...
      if (window.MemeManagerUI.state.imagePreviewState === state) {
        window.MemeManagerUI.emoji.renderImageSemantic(result?.semantic || {});
        window.MemeManagerUI.emoji.setImageSemanticEditing(false);
      }
      await window.MemeManagerUI.emoji.fetchEmojis();
      window.MemeManagerUI.dialogs.showToast(result?.message || "已恢复自动生成状态。", "success");
    } catch (error) {
      window.MemeManagerUI.dialogs.showToast(error?.message || String(error), "error", "恢复失败");
    } finally {
      window.MemeManagerUI.emoji.setImageSemanticSaving(false, window.MemeManagerUI.state.imagePreviewRestoreAutoBtn);
    }
  }
window.MemeManagerUI.emoji.confirmCurrentImageCategory = async function () {
    if (!window.MemeManagerUI.state.imagePreviewState || !window.MemeManagerUI.state.imagePreviewCategoryConfirmBtn) return;
    const previewState = window.MemeManagerUI.state.imagePreviewState;
    ...
    try {
      ...
      await window.MemeManagerUI.emoji.fetchEmojis();
      window.MemeManagerUI.dialogs.showToast("已保存人工确认；图片或分类变化后会自动失效。", "success");
    } catch (error) {
      window.MemeManagerUI.dialogs.showToast(error?.message || String(error), "error", "确认失败");
    } finally {
      window.MemeManagerUI.emoji.restoreButton(window.MemeManagerUI.state.imagePreviewCategoryConfirmBtn);
    }
  }
