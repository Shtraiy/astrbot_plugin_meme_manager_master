...
class EventHandlerMixin:
    ...
    def _build_emotion_llm_injection_suffix(self, event: AstrMessageEvent) -> str:
        ...
        if hasattr(event, "get_extra"):
            cached_lines = event.get_extra("meme_manager_master_emotion_context_lines")
    ...
    def _resolve_emotion_llm_model(self, event: AstrMessageEvent) -> str | None:
        ...
        cached_model = str(event.get_extra("meme_manager_master_reply_model") or "").strip()
        if cached_model:
            return cached_model
        ...
        if model:
            event.set_extra("meme_manager_master_reply_model", model)
    ...
    def _build_emotion_images_for_event(
        ...
        for emotion in emotions:
            ...
            try:
                ...
                logger.error(f"[meme_manager_master] 构建表情图片失败: {e}")
        ...
        try:
            ...
            for temp_file in temp_files:
                try:
                    ...
                    logger.error(f"[meme_manager_master] 清理兼容接口临时文件失败: {e}")
    ...
    async def _inject_meme_prompt_impl(
        ...
        if hasattr(event, "set_extra"):
            event.set_extra(
                "meme_manager_master_emotion_context_lines",
                self._collect_emotion_context_lines_from_request(req),
            )
            reply_model = str(getattr(req, "model", "") or "").strip()
            if reply_model:
                event.set_extra("meme_manager_master_reply_model", reply_model)
    ...
    async def _resolve_emotion_llm_provider_id(self, event: AstrMessageEvent) -> str:
        ...
        cached_id = str(event.get_extra("meme_manager_master_reply_provider_id") or "").strip()
        if cached_id:
            return cached_id
        ...
        if selected_id:
            ...
            if not callable(get_provider) or get_provider(selected_id) is not None:
                event.set_extra("meme_manager_master_reply_provider_id", selected_id)
                logger.info(
                    "[meme_manager_master] 情感模型未单独配置，本轮复用事件指定回复模型: %s",
                    selected_id,
                )
                return selected_id
            logger.warning(
                "[meme_manager_master] 事件指定的回复模型不存在，回退会话模型: %s",
                selected_id,
            )
        ...
        try:
            ...
            logger.warning(
                "[meme_manager_master] 情感模型未配置，且无法取得当前回复模型: %s",
                exc,
            )
            return ""

        if not provider_id:
            logger.warning("[meme_manager_master] 情感模型未配置，当前回复模型也不可用")
            return ""

        event.set_extra("meme_manager_master_reply_provider_id", provider_id)
        logger.info(
            "[meme_manager_master] 情感模型未单独配置，本轮复用会话回复模型: %s",
            provider_id,
        )
        return provider_id
    ...
    async def _resp_semantic_llm_impl(
        ...
        try:
            ...
            event.set_extra("meme_manager_master_semantic_query", query)
            if candidates:
                visible_candidates = [
                    {
                        "id": item.get("id"),
            ...
            logger.error(
                "[meme_manager_master] Emotion-assisted semantic search failed: %s",
                exc,
                exc_info=True,
            )
    ...
    async def _build_emotion_semantic_query(
        ...
        provider_id = await self._resolve_emotion_llm_provider_id(event)
        if not provider_id:
            logger.warning("[meme_manager_master] 无可用情感模型，使用截短后的可见回复检索")
            return fallback
        ...
        try:
            ...
            logger.warning(
                "[meme_manager_master] 生成短检索词失败，使用截短后的可见回复: %s",
                exc,
            )
            query = fallback

        logger.info(f"[meme_manager_master] 情感模型语义检索词（{len(query)}字）: {query}")
        return query
    ...
    async def _resp_impl(self, event: AstrMessageEvent, response: LLMResponse):
        ...
        semantic_mode = (
            str(event.get_extra("meme_manager_master_semantic_mode") or "")
            if self._semantic_mode_active(event)
            else ""
        )
        if bool(event.get_extra("meme_manager_master_semantic_response_processed")):
            logger.info(
                "[meme_manager_master] Response already processed; ignoring duplicate hook"
            )
            return
        ...
        event.set_extra("meme_manager_master_semantic_response_processed", True)
        ...
        logger.debug(f"[meme_manager_master] 重复检测阶段找到的表情: {repeated_emotions}")
        ...
        logger.debug(f"[meme_manager_master] 松散匹配阶段找到的表情: {loose_emotions}")
        ...
        if self.emotion_llm_enabled:
            try:
                ...
                raw_text = str(getattr(llm_resp, "completion_text", "") or "").strip()
                logger.debug(
                    "[meme_manager_master] 情感标签模型原始输出: %s",
                    raw_text[:500],
                )
                ...
                if selected_emotions:
                    ...
                    logger.info(
                        "[meme_manager_master] 情感模型选择标签: %s",
                        selected_emotions,
                    )
                else:
                    logger.info("[meme_manager_master] 情感模型未选择有效标签")
            except Exception as e:
                logger.error(f"[meme_manager_master] 情感模型调用失败: {e}")

        # 去重；严格数量限制由兼容配置决定。
        filtered_emotions = self._filter_emotion_selection(found_emotions)

        event.set_extra("found_emotions", filtered_emotions)
        logger.info(f"[meme_manager_master] 去重后的最终表情列表: {filtered_emotions}")

        # 防御性清理残留符号
        clean_text = re.sub(r"&&+", "", clean_text)  # 清除未成对的&&符号
        response.completion_text = clean_text.strip()
        logger.debug(
            f"[meme_manager_master] 清理后的最终文本内容长度: {len(response.completion_text)}"
        )
        ...
        if (
            ...
            try:
                ...
                logger.debug("[meme_manager_master] webchat 流式文本已替换为干净版本")
            except Exception as e:
                logger.error(f"[meme_manager_master] webchat 流式文本替换失败: {e}")
    ...
    async def _resp_semantic_impl(
        ...
        if (
            not selected_ids
            and str(event.get_extra("meme_manager_master_semantic_mode") or "") == "tool"
        ):
            default_id = str(
                event.get_extra("meme_manager_master_semantic_default_id") or ""
            ).strip()
            if default_id and validate_selected_id(
                ...
                logger.info(
                    "[meme_manager_master] 回复模型未显式选图，自动使用语义检索首候选: %s",
                    default_id,
                )
        if selected_ids:
            logger.info("[meme_manager_master] 本轮最终语义表情: %s", selected_ids[0])
        else:
            logger.warning("[meme_manager_master] 本轮语义检索没有可发送的有效候选")
        # 不让模型写出的畸形标记泄漏到用户消息中。
        clean_text = re.sub(r"&&\s*meme:[^&]+&&", "", clean_text, flags=re.IGNORECASE)
        event.set_extra("meme_manager_master_semantic_selected_ids", selected_ids)
    ...
    async def _on_decorating_result_impl(self, event: AstrMessageEvent):
        ...
        logger.debug("[meme_manager_master] on_decorating_result 开始处理")
        ...
        try:
            ...
            semantic_selected_ids = (
                event.get_extra("meme_manager_master_semantic_selected_ids") or []
            )
            if semantic_selected_ids and self._semantic_mode_active(event):
                ...
                if semantic_temp_files:
                    event.set_extra("meme_manager_master_temp_files", semantic_temp_files)
                if semantic_images:
                    # 语义模式不再随机丢弃模型已经选择的候选。
                    event.set_extra("meme_manager_master_pending_images", semantic_images)
            ...
            if found_emotions and not semantic_selected_ids:
                ...
                if random_value <= threshold:
                    ...
                    if emotion_images:
                        ...
                        if temp_files:
                            existing_temp_files = (
                                event.get_extra("meme_manager_master_temp_files") or []
                            )
                            event.set_extra(
                                "meme_manager_master_temp_files",
                        ...
                        if use_mixed_message:
                            ...
                            event.set_extra(
                                "meme_manager_master_pending_images", emotion_images
                            )
                    else:
                        pass
    ...
    async def _after_message_sent_impl(self, event: AstrMessageEvent):
        ...
        try:
            ...
            event.set_extra("meme_manager_master_pending_images", None)

            # 清理临时文件
            temp_files = event.get_extra("meme_manager_master_temp_files")
            if temp_files:
                for temp_file in temp_files:
                    try:
                        if os.path.exists(temp_file):
                            os.remove(temp_file)
                            logger.debug(f"[meme_manager_master] 已清理临时文件: {temp_file}")
                    except Exception as e:
                        logger.error(f"[meme_manager_master] 清理临时文件失败: {e}")
                event.set_extra("meme_manager_master_temp_files", None)
    ...
    def _convert_to_gif(self, image_path: str) -> str:
        ...
        try:
            with PILImage.open(image_path) as img:
                ...
                logger.debug(f"[meme_manager_master] 已将静态图转换为 GIF: {temp_filename}")
                return temp_filename
        except Exception as e:
            logger.error(f"[meme_manager_master] 转换图片为 GIF 失败: {e}")
            return image_path
        ...
        try:
            ...
            for emotion in found_emotions:
                ...
                try:
                    ...
                    logger.error(f"[meme_manager_master] 流式模式发送表情失败: {e}")
                finally:
                    # 清理临时文件
            ...
            logger.error(f"[meme_manager_master] 流式模式处理表情失败: {e}")
            logger.error(traceback.format_exc())
        finally:
            event.set_extra("found_emotions", None)
    ...
    async def _ensure_image_send_format(self, image: Image) -> Image:
        ...
        try:
            ...
            logger.error(f"[meme_manager_master] 转换图片为 base64 失败: {e}")
            return image
    ...
    def _merge_components_with_images(self, components, images):
        ...
        """
        logger.debug(
            f"[meme_manager_master] _merge_components_with_images 输入: 组件总数={len(components)}, 图片总数={len(images)}"
        )
        ...
        logger.debug(f"[meme_manager_master] Plain 组件的索引位置列表: {plain_indices}")
        ...
        for idx, plain_idx in enumerate(plain_indices):
            ...
            logger.debug(
                f"[meme_manager_master] Plain 组件 {idx} (索引={plain_idx}) 分配的图片数量: {images_for_this_text}"
            )
        ...
        logger.debug(
            f"[meme_manager_master] 合并前组件总数: {len(components)}, 合并后组件总数: {len(merged_components)}"
        )
